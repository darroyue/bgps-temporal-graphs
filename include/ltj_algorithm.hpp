        ...
        bool search(const size_type j, tuple_type &tuple, results_type &res,
            ...
            if(j == m_veo.size()){
                ...
                if( itrs.size() == 1 && itrs[0]->in_last_level()) {//Lonely variables
                    ...
                    // We now check whether we are at a time-related variable or not
                    if (itrs[0]->is_variable_time(x_j)) { 
                        ...
                        while (c.second != 0) { //If empty c.second==0
                            ...
                            for (ltj_iter_type* iter : itrs) {
                                iter->down(x_j, c.second-1); // We store only the second component of the interval
                                                             // in the stack m_path_label of the iterator
                            }
                            m_veo.down();
                            ...
                            c = seek_interval(x_j, c.second); // since interval c is semi open, no need for c.second+1
                            //cout << "Seek (loop): (" << (uint64_t) x_j << ": " << c << ")" <<endl;
                        }
                    }
                    else { 
