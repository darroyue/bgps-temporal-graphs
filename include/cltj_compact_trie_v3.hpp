    ...
    public:
        ...
        compact_trie_v3 &operator=(compact_trie_v3 &&o) {
            if (this != &o) {
                ...
                m_last_update_select1.set_vector(&m_last_update_per_int); // OJO
            }
            return *this;
        }
        ...
        */
        ...
        size_type get_last_update_of_interval(size_type pos) {
            //cout << "get_last_update_of_interval(), BV of :" << m_last_update_per_int.size() << " bits" << endl;
            //for (uint64_t i = 0; i < m_last_update_per_int.size(); i++)
            //    cout << m_last_update_per_int[i];
            //cout << endl;
            return m_last_update_select1(pos+1/*OJO con el +1*/);
        }

        std::pair<size_type, size_type> get_interval_at_pos(size_type pos) {
            //cout << "cltj_compact_trie_v3::get_interval_at_pos pos = " << pos << endl;
            //cout << "Va a ejecutar m_tempint_left[" << seq.size() - pos << "]" << endl;
            //cout << "Va a ejecutar m_tempint_right[" << seq.size() - pos << "]" << endl;
        ...
        */
        ...
        pair<temporal_interval, uint32_t> binary_search_interval_seek(uint32_t val, uint32_t i, uint32_t f) const {
            i = i - m_seq.size(); /*OJO*/
            f = f - m_seq.size(); /*OJO*/
            //cout << "binary_search_interval_seek() i = " << i << " f = " << f << " m_tempint_left.size() = " << m_tempint_left.size() << endl;
            //cout << "binary_search_interval_seek() [" <<  m_tempint_left[f] << ", " << m_tempint_right[f] << ")" << endl;
            //cout << "cltj_compact_trie_v3::binary_search_interval_seek() va a buscar el " << val << " en el segmento [" << i << ", " << f << "] de la lista de intervalos" << endl;
            //cout << "cltj_compact_trie_v3::binary_search_interval_seek() El intervalo en la posicion " << i << " es [" << m_tempint_left[i] << ", " << m_tempint_right[i] << "]" << endl;  
        ...
        /*
        size_type temporal_successor_aux(size_type depth, std::pair<size_type, size_type> &node_interval, 
                                     int64_t pos, value_type val,
                                     std::pair<std::pair<size_type, size_type>, size_type> &node_pair
                                    ) {            
            return m_temporal_ds.leap_aux(depth*m_temporal_ds.get_n_bits(), (int64_t)node_interval.first, 
                                          (int64_t)node_interval.second,
                                          (int64_t)pos, val, m_temporal_ds.get_n_bits(), node_pair);
        }
        */
        ...
        size_type temporal_successor(size_type depth, std::pair<size_type, size_type> &node_interval, 
                                     /*size_type*/ int64_t pos, value_type val,
                                     std::pair<std::pair<size_type, size_type>, size_type> &node_pair
                                    ) {            
            return m_temporal_ds.leap(depth*m_temporal_ds.get_n_bits(), (int64_t)node_interval.first, (int64_t)node_interval.second,
                                      (int64_t)pos, val, m_temporal_ds.get_n_bits(), node_pair);
        }
        ...
        void load(std::istream &in) {
            //cout << "En compact_trie_v3::load()" << endl;
            uint64_t i = 1;
            //cout << i++ << endl;
