...
namespace ltj {
    ...
    namespace veo {
        ...
        public:
            ...
            veo_simple(const vector<quad_pattern> *quad_patterns,
                ...
                size_type i = 0;
                for (const quad_pattern &quad_pattern : *m_ptr_quad_patterns) {
                    bool s = false, p = false, o = false, t = false;
                    var_type var_s, var_p, var_o, var_t;
                    size_type size;
                    if (quad_pattern.s_is_variable()) {
                        s = true;
                        var_s = (var_type) quad_pattern.term_s.value;
                        size = veo_trait_type::subject(m_ptr_iterators->at(i));
                        var_to_vector(var_s, size, hash_table_position, var_info);
                    }
                    if (quad_pattern.p_is_variable()) {
                        p = true;
                        var_p = (var_type) quad_pattern.term_p.value;
                        size = veo_trait_type::predicate(m_ptr_iterators->at(i));
                        var_to_vector(var_p, size, hash_table_position, var_info);
                    }
                    if (quad_pattern.o_is_variable()) {
                        o = true;
                        var_o = (var_type) quad_pattern.term_o.value;
                        size = veo_trait_type::object(m_ptr_iterators->at(i));
                        var_to_vector(var_o, size, hash_table_position, var_info);
                    }
                    if (quad_pattern.t_is_variable()) {
                        t = true;
                        var_t = (var_type) quad_pattern.term_t.value;
                        size = veo_trait_type::time(m_ptr_iterators->at(i));
                        var_to_vector(var_t, size, hash_table_position, var_info);
                    }
                    
                    // OJO: a continuacion me falta la alternativa de t, consultar cual es la idea de lo que sigue
                ...
                /*for (uint64_t d = 0; d < m_order.size(); d++)
                    cout << (uint64_t)m_order[d] << endl; */
                //cout << "Done. " << endl;
            }
