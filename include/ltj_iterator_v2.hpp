...
namespace ltj {
    ...
    private:
        ...
        void process_constants() {
           ...
           //cout << "process_constants() done" << endl; 
        }

        void choose_trie_old(state_type state) {
            if(m_nfixed == 0) {
                ...
                m_switch = false; // no trie switch needed at this level
                if (state == s) { //Fix variables
                    switch (m_fixed[m_nfixed-1]) {
                        case o: m_trie_i = OSP;
                                m_status_i = 1; // OJO
                                m_switch = false; // at node OS, no switch 
                                break;
                        ...
                        case t: m_trie_i = TSPO;
                                m_status_i = 0; // OJO con esto, creo que no hay que hacer switch en este caso  
                                m_switch = false;
                                break; 
                    }
                } else if (state == p) {
                    switch (m_fixed[m_nfixed-1]) {
                        case s: m_trie_i = SPOT;
                                m_status_i = 1; // at node SP, with 2 children in metatrie
                                m_switch = false;
                                break;
                        case o: m_trie_i = POS;
                                m_status_i = 1; // at node PO, which has two children in the metatrie
                                m_switch = true;
                                break;
                        case t: m_trie_i = TPOS;
                                m_status_i = 1; //1; // OJO
                                m_switch = false;  // No hay trie switching aqui, simplemente me cambio de trie por el metatrie
                                break;
                    }
                } else if (state == o) {
                    switch(m_fixed[m_nfixed-1]) {
            ...
            //if (m_status_i != 0) m_status_i = 0;
        }

        void choose_trie_new(state_type state) {
            if(m_nfixed == 0) {
                // at depth 0 in the trie, so we are going down by the first state
                m_trie_i = 4*state;   //OJO con el 4, antes era 2
                m_status_i = 0;
                ...
                if (state == s) { //Fix variables
                    ...
                    switch (m_fixed[m_nfixed-1]) {
                        case s: m_trie_i = SPOT;
                                m_status_i = 0; // at node SP, with 2 children in metatrie
                                m_switch = false;
                                break;
                        case o: m_trie_i = POS;
                                m_status_i = 2; // at node PO, which has two children in the metatrie
                                m_switch = true;
                                break;
                        case t: m_trie_i = TPOS;
                                m_status_i = 2; //1; // OJO
        ...
        void choose_trie_leap(state_type state, size_type &t_i, size_type &s_i) {
            ...
            if(m_nfixed == 0) {
                // at depth 0 in the trie, so we are going down by the first state
                t_i = 4*state;   //OJO con el 4, antes era 2
                s_i = 0;
                //m_switch = false;
            } else if (m_nfixed == 1) {
                // at depth 1 in the trie, a single state is already fixed
        ...
        */
        ...
        void down(state_type state) {
            ...
            if (at_time_level()) {
                ...
                //cout << "En down() we are now m_below_time_level = true" << endl;
            }
            ...
            if(m_nfixed == 1) {
                if (state != t) {
                    ...
                    // we are in time-first, so currently m_trie_i = SPOT
                    // Estaremos todo el tiempo en la estructura temporal
                    // por lo que parent() no necesita ser usado ya que en esta
                    // estructura todo se maneja con un intervalo dentro de la matriz
                    // y con una posicion, lo cual se maneja con m_status[].beg, end, y cnt.
                    // Notar que todo lo que se hace con las otras variables es para poder
                    // implementar parent(), creo, revisar
                    const auto* trie = m_ptr_index->get_trie(m_trie_i);
                    ...
                    m_status[m_nfixed].it[3] = 0; // OJO deadline
                ...
                if (m_nfixed == 2) {
                    ...
                    // Aqui debo empezar a preguntar si hago m_switch o no,
                    // Por ejemplo, si estoy bajando a SO, inmediatamente hago switch a OS
                    // Lo tengo que hacer en este punto para que funcione, para que el leap
                    // pueda bajar desde alla. Hay que modificar eso, porque estoy haciendo
                    // el switch tarde (lo estoy haciendo a partir de m_fixed == 3). 
                    if (m_switch /*!m_status_i*/) { 
                        // We should be in SO, ST, PS, PT, OP, OT, so trie switch
                        // in this case m_status_i indicates whether there is trie switch or not 
                        size_type switch_node = trie_switch_down(m_nfixed, state);
                        //m_status[m_nfixed].it[m_status_i] = switch_node;
                        // Warning: verificar que no necesite guardar mas cosas en otros iteradores, me parece que no
                    } else {  
                        // meta trie when instantiating the second component in a regular trie
                        ...
                        /*if (m_status_i) {*/
                            ...
                            m_status[m_nfixed].it[1] = trie_aux->nodeselect(m_status[m_nfixed].beg-trie->root_degree()-1); // -1 as there is no root in this trie
                        /*} else {
                            const auto* trie = m_ptr_index->get_trie(m_trie_i);
                            auto pos = m_status[m_nfixed].beg;
                            m_status[m_nfixed].it[0] = trie->nodeselect(pos);
                    ...
                    if (state == t) {
                        ...
                        size_type switch_node = trie_switch_down(m_nfixed, state);
                        // WARNING, se supone que me va a retornar un 0, es un dummy, el descenso
                        // deberia haberlo hecho el trie switch. Revisar esto.
                    } else {
                        ...
                        if (m_switch) {
                            ...
                            // SPO, PSO
                            // we are moving to a node of degree 0 in the metatrie, so no need to store multiple nodes in "it"
                            const auto* trie = m_ptr_index->get_trie(m_trie_i);
                            auto pos = m_status[m_nfixed].beg;
                            m_status[m_nfixed].it[0] = trie->nodeselect(pos);
                        }
                ...
                if (m_switch) {
                    ...
                    m_status[m_nfixed].it[0] = node_pair.second; //m_status[m_nfixed].beg;
                    m_status[m_nfixed].it[1] = node_pair.first.first; //m_status[m_nfixed].cnt;
                    m_status[m_nfixed].it[2] = node_pair.first.second; //m_status[m_nfixed].end;
                    m_status[m_nfixed].it[3] = 0; // OJO deadline
                }
            }
            //cout << "leaving down(state)" << endl;
        }
        ...
        std::pair<value_type, value_type> leap(var_type var, size_type c = -1ULL) { //Return the minimum in the range
            ...
            if(m_redo[m_nfixed]) { //First time of leap
                /*OJO con esto*/
                // WARNING, revisar que esto este bien
                ...
                if (m_nfixed == 0 && state == t) {
                    ...
                    // we are in a regular trie
                    // WARNING: time levels that are not time-first orders must pass over here, CHECK it WORKS CORRECTLY!
                    ...
                    // we are in a temporal data structure, m_nfixed >= 1 (creo que el igual vale)
                    if (m_nfixed == 1) {
                        // first level of the temporal data structure
                        beg = m_status[m_nfixed].it[0]; //trie->get_last_update_of_interval(m_status[m_nfixed].beg); 
                        auto cnt = m_status[m_nfixed].it[1];//node.first; 
                        end = m_status[m_nfixed].it[2];//node.second;
                        m_status[m_nfixed+1].beg = beg;
                        m_status[m_nfixed+1].end = end;
                        m_status[m_nfixed+1].cnt = cnt;
                        m_redo[m_nfixed] = false;
                    } else {
                        // Estoy en la estructura temporal, en el nivel > 1 (alguno de los 2 ultimos niveles)
                        //beg = m_status[m_nfixed].it[0];//beg = m_status[m_nfixed].beg;
                        //std::pair<size_type, size_type> cur_node = make_pair(m_status[m_nfixed].it[1], m_status[m_nfixed].it[2]);
                        //std::pair<std::pair<size_type, size_type>, size_type> node_pair;
                        //value_type succ = trie->temporal_successor(m_nfixed-1, cur_node, beg, 0 /*OJO aca, o debo buscar c?*/, node_pair);
                        beg = m_status[m_nfixed].it[0]; //node_pair.second;
                        auto cnt = m_status[m_nfixed].it[1]; //node_pair.first.first;
                        end = m_status[m_nfixed].it[2]; //node_pair.first.second;
                        m_status[m_nfixed+1].beg = beg;
                        m_status[m_nfixed+1].end = end;
                        m_status[m_nfixed+1].cnt = cnt;
                        m_redo[m_nfixed] = false;
            ...
            //cout << "Leap() c = " << c << " beg = " << beg << " end = " << end << endl;
            size_type value;
            if (c == -1ULL) {
                // Aca tengo que ver todos los casos para determinar como saco el primer
                // elemento del nodo actual
                if (below_time_level_leap) {
                    //cout << "Leap() va por la rama temporal" << endl;
                    // sucesor en la estructura temporal para sacar el primer elemento
                    std::pair<size_type, size_type> cur_node = make_pair(m_status[m_nfixed].it[1], m_status[m_nfixed].it[2]);
                    std::pair<std::pair<size_type, size_type>, size_type> node_pair;
                    // Now it looks for the successor of 0 in the current node
                    uint64_t init_value =  m_status[m_nfixed].it[3]; // succ0;  // OJO deadline
                    uint64_t s = 0;
                    if (m_nfixed == 2 && (t_i == TOP || t_i == TPS || t_i == TSO))
                        s = 1;

                    value = trie->temporal_successor(m_nfixed-1-s, cur_node, beg, init_value, node_pair);
                    m_status[m_nfixed].it[3] = value+1; // OJO deadline
                } else if (state == t) {
                    //cout << "Leap() va por la rama intervalistica" << endl;
                    auto value = trie->get_interval_at_pos(beg);
                    m_status[m_nfixed+1].beg = beg;
                    return value; // returns the interval
                } else {
                    //cout << "Leap() va por la rama normal" << endl;
                    value = trie->seq[beg];
                    m_status[m_nfixed+1].beg = beg; //First position in the sequence
                }
            } else {
                // aqui tengo que ver todos los casos para hacer la busqueda binaria
                if (below_time_level_leap/*m_below_time_level*/) {
                    std::pair<size_type, size_type> cur_node = make_pair(m_status[m_nfixed].it[1], m_status[m_nfixed].it[2]);
                    std::pair<std::pair<size_type, size_type>, size_type> node_pair;
                    uint64_t s = 0;
                    if (m_nfixed == 2 && (t_i == TOP || t_i == TPS || t_i == TSO))
                    ...
                    //cout << "Ahora en esta rama va a buscar por c = " << c << " en el intervalo [" << beg << ", " << end << "] de m_trie_i = " << m_trie_i << endl;
                    auto p = trie->binary_search_interval_seek(c, beg, end);
                    if (p.second > end) return make_pair(0, 0); // cuando es busqueda sobre los intervalos, [0,0) es la respuesta fallida
                    //cout << "Leap() por retornar el intervalo [" << p.first.first << ", " << p.first.second << "], beg = " << beg << " m_nfixed = " << m_nfixed << endl;
        ...
        std::pair<value_type, value_type> leap_constant(state_type state, size_type c = -1ULL) { //Return the minimum in the range
            ...
            const auto* trie = m_ptr_index->get_trie(t_i/*m_trie_i*/);
            //cout << "En Leap() eligio el trie " << (uint64_t)m_trie_i << endl;
            ...
            if(m_redo[m_nfixed]) { //First time of leap
                ...
                if (m_nfixed == 0 && state == t) {
                    ...
                    m_status[m_nfixed+1].cnt = end-beg+1;
                    m_redo[m_nfixed] = false;
                } else if (!below_time_level_leap/*m_below_time_level*/) {
                    // we are in a regular trie
                    // WARNING: time levels that are not time-first orders must pass over here, CHECK it WORKS CORRECTLY!
               ...
               // std::cout << "Current: " << current() << std::endl;
                beg = m_status[m_nfixed+1 /*OJO con el +1 aca y en los otros casos*/].beg;
                end = m_status[m_nfixed+1].end;
                //cnt = m_status[m_nfixed+1].cnt;
            }
            //cout << "Leap() c = " << c << " beg = " << beg << " end = " << end << endl;
            size_type value;
            if (c == -1ULL) {
                ...
                if (below_time_level_leap/*m_below_time_level*/) {
                    ...
                    m_status[m_nfixed].it[3] = value + 1; // succ // OJO deadline
                    
                } else if (state == t) {
                    //cout << "Leap() va por la rama intervalistica" << endl;
                    auto value = trie->get_interval_at_pos(beg);
                    //cout << "Leap() por retornar el intervalo [" << value.first << ", " << value.second << "], beg = " << beg << " m_nfixed = " << m_nfixed << endl;
                    //cout << "m_fixed = " << m_fixed[0] << ", " << m_fixed[1] << ", " << m_fixed[2] << endl; 
                    //auto p = trie->binary_search_interval_seek(c, beg, end);
                    m_status[m_nfixed+1].beg = beg;
                    return value; // returns the interval
                } else {
                ...
                if (m_below_time_level) {
                    // WARNING, revisar el valor de beg aqui.
                    std::pair<size_type, size_type> cur_node = make_pair(m_status[m_nfixed].it[1], m_status[m_nfixed].it[2]);
                    std::pair<std::pair<size_type, size_type>, size_type> node_pair;
                    // Now it looks for the successor of c in the current node
        ...
        bool at_time_level() {
            //cout << "at_time_level(): m_nfixed-1 = " << m_nfixed-1 << " m_fixed[m_nfixed-1] = " << m_fixed[m_nfixed-1] << endl;
            if (m_nfixed == 0) return false;
            //cout << m_fixed[0] << " " << m_fixed[1] << " " << m_fixed[2] << endl; 
            return m_fixed[m_nfixed-1] == t;
        }

        inline size_type children(state_type state) const { 
            size_type t_i, s_i=0;
            // cout << "At ltj_iterator_v2::children() s_i = " << s_i << endl;
            // cout << "At ltj_iterator_v2::children() m_fixed[0] = " << m_fixed[0] << " m_fixed[1] = " << m_fixed[1] << " m_fixed[2] = " << m_fixed[2] << endl;
            // cout << "At ltj_iterator_v2::children() m_trie_i = " << m_trie_i << " m_nfixed = " << m_nfixed << endl;
            ...
            if (t_i < TSPO) {
                ...
                //cout << "DEGREE em temporales m_nfixed = " << m_nfixed << endl;
                if (m_nfixed == 0) {
                    auto trie = m_ptr_index->get_trie(SPOT);
                    return trie->number_of_updates(); //trie->root_degree();
                } else {
                    //if (m_nfixed == 2 && (t_i == TSO || t_i == TPS || t_i == TOP))
                    auto trie = m_ptr_index->get_trie(t_i);
