...
ltj::quad_pattern get_quad(string & s, std::unordered_map<std::string, uint8_t> &hash_table_vars) {
    vector<string> terms = tokenizer(s, ' ');

    ltj::quad_pattern quad;
    if(is_variable(terms[0])) {
        quad.var_s(get_variable(terms[0], hash_table_vars));
    } else {
        quad.const_s(get_constant(terms[0]));
    }
    if(is_variable(terms[1])) {
        quad.var_p(get_variable(terms[1], hash_table_vars));
    } else {
        quad.const_p(get_constant(terms[1]));
    }
    if(is_variable(terms[2])) {
        quad.var_o(get_variable(terms[2], hash_table_vars));
    } else {
        quad.const_o(get_constant(terms[2]));
    }
    if(is_variable(terms[3])) {
        quad.var_t(get_variable(terms[3], hash_table_vars));
    } else {
        quad.const_t(get_constant(terms[3]));
    }
    return quad;
}
...
void query(const std::string &file, const std::string &queries, const uint64_t limit){
    ...
    cout << "En query() por cargar el indice" << endl;
    ...
    if(result)
    {
        ...
        for (string& query_string : dummy_queries) {
            ...
            std::vector<ltj::quad_pattern> query;
            vector<string> tokens_query = tokenizer(query_string, '.');
            for (string& token : tokens_query) {
                auto quad_pattern = get_quad/*triple*/(token, hash_table_vars);
                query.push_back(quad_pattern);
            }
...
int main(int argc, char* argv[])
{
    ...
    cout << "En main() antes de llamar a query()" << endl;
    query<cltj::compact_temporal_ltj_metatrie, ltj::util::trait_distinct>(index, queries, limit); // OJO con esto!
